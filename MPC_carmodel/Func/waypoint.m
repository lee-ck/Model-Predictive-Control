...
px = [Map.points(1,1) Map.points(1,:) Map.points(1,end)];
py = [Map.points(2,1) Map.points(2,:) Map.points(2,end)];
pp = spcrv([px;py],3);
a= 0;
for i = 1:size(pp,2)-1
a = a+ norm(pp(:,i+1)-pp(:,i));
end
target_dist = sim.target_velocity*sim.simTs;
num = fix(a/target_dist);
